...
def handle_page(page,asset,num,exp):
    with open(os.path.join(exp+".csv"),'wb') as csvfile:
        writer=csv.DictWriter(csvfile,fieldnames=["id"], delimiter=',')
        writer.writeheader()
    open(os.path.join(exp+".txt"), 'w')
    if num<250:
        n=0
        for items in page['features']:
            for itm in items['_permissions']:
                if itm.split(':')[0]=="assets."+asset and n<num:
                    ...
                    print(it)
                    n=n+1
                    with open(os.path.join(exp+".csv"),'a') as csvfile:
                        writer=csv.writer(csvfile,delimiter=',',lineterminator='\n')
                        writer.writerow([it])
                    with open(os.path.join(exp+".txt"), 'a') as the_file:
                        the_file.write(it+'\n')
        sys.exit()
    else:
        n=0
            ...
            for itm in items['_permissions']:
                if itm.split(':')[0]=="assets."+asset and n<num:
                    ...
                    with open(os.path.join(exp+".csv"),'a') as csvfile:
                        writer=csv.writer(csvfile,delimiter=',',lineterminator='\n')
                        writer.writerow([it])
        data=csv.reader(open(os.path.join(exp+".csv")).readlines()[1: num])

        with open(os.path.join(exp+".csv"), "wb") as f:
            writer = csv.writer(f)
            for row in data:
                writer.writerow(row)
        open(os.path.join(exp+".txt"), 'w')
        dat=open(os.path.join(exp+".csv")).readlines()
        with open(os.path.join(exp+".txt"), 'a') as the_file:
            for row in dat:
                the_file.write(row)
...
def idl(infile,item,asset,num):
    ...
    exp=os.path.join(head,tail.split('.')[0])
...
## Send post request
    result = requests.post('https://api.planet.com/data/v1/quick-search',
                           ...
                           auth=(PL_API_KEY, ''))
    page=result.json()
    final_list = handle_page(page,asset,num,exp)
...
if len(sys.argv)==5:
    idl(infile=os.path.normpath(sys.argv[1]),item=sys.argv[2],asset=sys.argv[3],num=int(sys.argv[4]))
