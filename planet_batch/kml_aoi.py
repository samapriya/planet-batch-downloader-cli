...
def parsed(args):
        ...
        with open(os.path.join(os.path.join(os.path.splitext(args.loc)[0],os.path.splitext(os.path.basename(args.geo))[0]+"_aoi.json")), 'w') as outfile:
            outfile.write(filenames)
            outfile.close()
        return "Processed "+str(os.path.basename(kml_file))+" and "
